...
- **Experiment (pre-adoption): per-fingerprint stratification of B1 calibration.**
  ...
  **Step 1 RESULT (run 2026-07-07,
  `scripts/ensemble/eda_fingerprint_stratification.py`; full write-up in
  `data/output/data_investigation_results.md`).** 875 decided verdicts -> 420
  (rule, cik) groups: median group n=1 (240 singletons), 42 groups n>=5, 6 groups
  n>=10 — census per-group calibration is unreachable. Dispersion is real but
  concentrated: FX01 p~5e-5 (survives any correction; two CIKs at ~100% real vs
  pooled 0.51 carry 27% of sampled FX01 flagged units), PP03 p=0.017;
  GAV_BDC02/PCT01 marginal. Heterogeneity is per-FILER (0001803498 ~100% real across
  FX01+PP03; 0002031750 across A07+X08). Decision per the rule above: adopt the cheap
  form only — record fingerprint + pass/baseline stamp on every future B1 sample; no
  per-group sampling infrastructure. FX01's pending deterministic gate supersedes its
  routing prior anyway.

  **Step 2 (prospective; ordered AFTER gap 1 — see 8.1).** Getting per-group density
  where it matters does not need ~10.5k census verdicts (420 groups x 25); it needs:
  (i) the stamp above (costless, adopted); (ii) per-group MINIMUM QUOTAS in the first
  post-gap-1 recalibration batch for the divergent candidates (~5-10 groups x ~25 =
  150-250 verdicts, one modest batch that double-duties as the fresh-pool per-rule
  recalibration); (iii) the printed-cell gate as the verdict machine at scale — every
  quote-at-location check is a real/FA label at near-zero marginal cost, so
  field-faithfulness rules get per-group density free from acceptance samples and
  no_defect checks, leaving B1's scarce verdicts exclusively for the gate-less rules
  (PP03, B01, GAV_BDC02, FX01-until-gated). Era-windowing is mandatory: verdicts
  describe the firing pool as of their pass; estimates window to the current era
  rather than averaging across promotion waves. Do NOT draw new B1 batches before the
  first post-gap-1 rebuild — they would spend the scarcest resource adjudicating
  flags the pending fixes already erase.
...
**Execution order vs B1 sampling (decided 2026-07-07): gap 1 BEFORE any new B1
batch.** B1 verdicts describe the firing pool as of when they are drawn; a validated
backlog (the overnight 41-CIK gate-PASS rules, the anchor overrides) is waiting to
change that pool. Batches drawn now would partly adjudicate flags a rebuild erases and
go stale the day the first wave applies. The first new B1 batch is the recalibration
that pass 1 triggers anyway, with the section 5 step-2 per-group quotas folded in —
fresh per-rule priors on the new pool and per-group density in one batch, zero wasted
verdicts.
