...
interface FundHoldingsTabProps {
  holdings: FundHolding[] | null;
}

export default function FundHoldingsTab({ holdings }: FundHoldingsTabProps) {
  ...
  return (
    <div className="py-6">
          ...
          <tbody>
            {holdings.map((h, i) => (
              ...
              >
                ...
                {showMaturity && (
                  <td className="py-2.5 px-4 text-right tabular-nums text-ink3 text-xs">
                    {h.maturityDate ?? '--'}
                  </td>
                )}
      ...
      <p className="text-xs text-ink3 mt-3">
        {holdings.length} holdings shown
      </p>
    </div>
  );
}
